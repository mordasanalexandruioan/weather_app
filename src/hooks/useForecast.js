...
const BASE_URL='http://api.weatherapi.com/v1/forecast.json?key=b6d505ab68f4411e8ab124145222505';
...
const useForecast = () =>{
    ...
    const getWoeid = async (location)=>{
        const { data } = await axios(`${BASE_URL}`, {params: {query: location, days: 3, aqi:'yes', alerts: 'yes'}});
    ...
    const getherForecastData=(data)=>{
      ...
      setForecast({currentDay, currentDayDescr});
      setLoading(false);
    }
    ...
    const submitRequest = async location=>{
        try {
            ...
            const response = await axios(`${BASE_URL}`, {params: {query: location, days: 3, aqi:'yes', alerts: 'yes'}});
            setLoading(false);
